...
class postprocess(object):
    ...
    def sublanguage(self):
        ...
        if self.get_all_subtitles:
            for subfix in self.subfixes:
                ...
                #  words back to swedish like oversattning->översättning? or rewrite the subtitle class to take the title directly from the service?
        ...
        if len(langs) >= 2:
            ...
            log.info("Subtitles language codes: " + ', '.join(lang_codes))
        else:
            log.info("Subtitle language code: " + langs[0]['lang'])
        return langs
    ...
    def remux_mkv(self):
        ...
        if self.detect_mkvmerge:
            ...
            log.debug("Can't detect mkvmerge, using ffmpeg/avconv instead")
    ...
    def remux_mkv_ffmpeg(self):
        ...
        if self.detect_ffmpeg is None:
            log.error("Cant detect ffmpeg or avconv. Can't mux files without it.")
            return
        ...
        if self.stream.options.output.endswith('.mkv') is False:
            ...
            if ext == '.ts':
                log.info('Changing bitstream for TS audio by muxing to MP4')
                self.remux_mp4()
            ...
            if self.merge_subtitle:
                log.info(u"Muxing {0} and merging its subtitle into {1} using ffmpeg/avconv".format(orig_filename, new_name))
            else:
                log.info(u"Muxing {0} into {1} using ffmpeg/avconv".format(orig_filename, new_name))
            tempfile = u"{0}.temp".format(orig_filename)

            arguments = ["-map", "0:{}".format(videotrack), "-map", "0:{}".format(audiotrack), "-c", "copy", # "-copyts",
                         "-f", "matroska"]
            if ext == ".ts":
    ...
    def remux_mkvmerge(self):
        if self.detect_mkvmerge is None: # this check will probably never be False  if called by remux()
            log.error("Cant detect mkvmerge. Can't mux into matroska file without it.")
            return
        ...
        if self.merge_subtitle:
            log.info(u"Muxing {0} and merging its subtitle into {1} using mkvmerge".format(orig_filename, new_name))
        else:
            log.info(u"Muxing {0} into {1} using mkvmerge".format(orig_filename, new_name))
    ...
    def remux_mp4(self):
        ...
        if self.stream.options.output.endswith('.mp4') is False:
            ...
            if self.merge_subtitle and not self.mkv:
            ...
            tempfile = u"{0}.temp".format(orig_filename)
            arguments = ["-map", "0:{}".format(videotrack), "-map", "0:{}".format(audiotrack), "-c", "copy", "-f", "mp4"] # "-copyts"
            if ext == ".ts":
                arguments += ["-bsf:a", "aac_adtstoasc"]
            ...
            if self.merge_subtitle and not self.external_subtitle and not self.mkv:
                log.info("Muxing done, removing the old files.")
                for lang in langs:
                    os.remove(lang['sub_file'])
            else:
    ...
    def merge_mkv(self):
        ...
        if self.detect_mkvmerge:
            ...
            log.info("Can't detect mkvmerge, using ffmpeg/avconv instead")
    ...
    def merge_mkv_mkvmerge(self):
        ...
        if ext == '.ts':
            log.info('Chaning bitstream for TS audio with ffmpeg/avconv')
    ...
    def merge_mkv_ffmpeg(self):
        ...
        if self.merge_subtitle:
            log.info("Merge audio, video and subtitle into {0}".format(new_filename))
        else:
            log.info("Merge audio and video into {0}".format(new_filename))
    ...
    def merge_mp4(self):
        ...
        orig_filename = self.stream.options.output
        name, ext = os.path.splitext(orig_filename)
        new_filename = u"{0}.mp4".format(name)
    ...
    def _clean_ts_audio(self):
        ...
        log.info('Fixing bitstream for %s', audio_filename)
        cmd = [self.detect_ffmpeg, "-i", audio_filename]
