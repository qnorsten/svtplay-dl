...
class postprocess(object):
    ...
    def remux_mkv(self):
        ...
        if self.detect_mkvmerge: # and self.config.get('prefered_mkv_tool') == 'ffmpeg': # todo add something similar to this to allow config to overwrite priority
    ...
    def remux_mkv_mkvmerge(self):
        ...
        if ext == '.ts':
            log.info('Changing bitstream for TS audio by muxing to MP4 first')
            self.remux_mp4()
            orig_filename = u"{0}.mp4".format(name)
    ...
    def _clean_ts_audio(self):
        ...
        log.info("Audio cleaning done, removing old files.")
        os.remove(audio_filename)
